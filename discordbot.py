"""
Discord Bot - 低位株分析Bot（予測追跡・精度改善機能付き）

コマンド一覧:
  /stock <コード>       - 個別銘柄を分析（例: /stock 7201.T）
  /screen               - 低位株の本日注目TOP5を表示・予測保存
  /reversal             - 反転シグナル検出銘柄を表示・予測保存
...
async def run_daily_report(channel, loop=None):
    ...
    top = await loop.run_in_executor(
        None,
        lambda: screen_low_price_stocks(
            custom_watchlist, max_price=1000, top_n=5, weights=weights
        ),
    )
    reversal = await loop.run_in_executor(
        None,
        lambda: screen_reversal_stocks(
            custom_watchlist, max_price=1000, top_n=5, weights=weights
        ),
    )
    ...
    # 予測を保存（重複防止: 同日同銘柄は1件まで）
    seen = set()
    for a in top + reversal:
